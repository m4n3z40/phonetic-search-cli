...
function findAllMatches(wordsToMatch, dict) {
    return wordsToMatch.map(function (word) {
        return {
            word: word,
            matches: (0, _findMatches2.default)(word, dict)
        };
    }).filter(function (item) {
        return item.matches !== null;
    }).reduce(function (item, matches) {
        return matches[item.word] = item.matches;
    }, {});
}
...
function outputMatches(matches) {
    var matchedWords = Object.keys(matches);
    ...
    matchedWords.forEach(function (match) {
        return console.log(match + ': ' + matches[match].join(','));
    });
}